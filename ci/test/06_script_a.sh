...
if [ -n "$ANDROID_TOOLS_URL" ]; then
  ...
  CI_EXEC ./autogen.sh
  CI_EXEC ./configure "$WIDECOIN_CONFIG_ALL" "$WIDECOIN_CONFIG" || ( (CI_EXEC cat config.log) && false)
...
CI_EXEC "${BASE_ROOT_DIR}/configure" --cache-file=config.cache "$WIDECOIN_CONFIG_ALL" "$WIDECOIN_CONFIG" || ( (CI_EXEC cat config.log) && false)

CI_EXEC make distdir VERSION="$HOST"

export P_CI_DIR="${BASE_BUILD_DIR}/widecoin-$HOST"

CI_EXEC ./configure --cache-file=../config.cache "$WIDECOIN_CONFIG_ALL" "$WIDECOIN_CONFIG" || ( (CI_EXEC cat config.log) && false)
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  CI_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/widecoin-config.h > src/config/widecoin-config.h.tmp && mv src/config/widecoin-config.h.tmp src/config/widecoin-config.h'
fi
