...
(
  ...
  # See https://github.com/widecoin/widecoin/pull/28359#issuecomment-1698694748
...
if [ "$RUN_FUZZ_TESTS" = "true" ]; then
  ...
  if [ ! -d "$DIR_FUZZ_IN" ]; then
    ${CI_RETRY_EXE} git clone --depth=1 https://github.com/widecoin-core/qa-assets "${DIR_QA_ASSETS}"
  fi
  (
    cd "${DIR_QA_ASSETS}"
  ...
  if [ ! -d "$DIR_UNIT_TEST_DATA" ]; then
    ...
    ${CI_RETRY_EXE} curl --location --fail https://github.com/widecoin-core/qa-assets/raw/main/unit_test_data/script_assets_test.json -o "${DIR_UNIT_TEST_DATA}/script_assets_test.json"
  fi
fi
...
WIDECOIN_CONFIG_ALL="--disable-dependency-tracking"
if [ -z "$NO_DEPENDS" ]; then
...
if [ -n "$ANDROID_TOOLS_URL" ]; then
  make distclean || true
  ./autogen.sh
  bash -c "./configure $WIDECOIN_CONFIG_ALL $WIDECOIN_CONFIG" || ( (cat config.log) && false)
...
bash -c "${BASE_ROOT_DIR}/configure --cache-file=config.cache $WIDECOIN_CONFIG_ALL $WIDECOIN_CONFIG" || ( (cat config.log) && false)

make distdir VERSION="$HOST"

cd "${BASE_BUILD_DIR}/widecoin-$HOST"

bash -c "./configure --cache-file=../config.cache $WIDECOIN_CONFIG_ALL $WIDECOIN_CONFIG" || ( (cat config.log) && false)
...
if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
  bash -c "${TEST_RUNNER_ENV} DIR_UNIT_TEST_DATA=${DIR_UNIT_TEST_DATA} LD_LIBRARY_PATH=${DEPENDS_DIR}/${HOST}/lib ${BASE_OUTDIR}/bin/test_widecoin --catch_system_errors=no -l test_suite"
fi
...
if [ "${RUN_TIDY}" = "true" ]; then
  cmake -B /tidy-build -DLLVM_DIR=/usr/lib/llvm-"${TIDY_LLVM_V}"/cmake -DCMAKE_BUILD_TYPE=Release -S "${BASE_ROOT_DIR}"/contrib/devtools/widecoin-tidy
  cmake --build /tidy-build "$MAKEJOBS"
  cmake --build /tidy-build --target widecoin-tidy-tests "$MAKEJOBS"

  set -eo pipefail
  cd "${BASE_BUILD_DIR}/widecoin-$HOST/src/"
  ( run-clang-tidy-"${TIDY_LLVM_V}" -quiet -load="/tidy-build/libwidecoin-tidy.so" "${MAKEJOBS}" ) | grep -C5 "error"
  ...
  python3 "/include-what-you-use/iwyu_tool.py" \
           -p . "${MAKEJOBS}" \
           -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_BUILD_DIR}/widecoin-$HOST/contrib/devtools/iwyu/widecoin.core.imp" \
