...
BOOST_AUTO_TEST_CASE(srd_tests)
{
...

    {
        ...
        const auto& res = SelectCoinsSRD(target, dummy_params, m_node, max_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 60; ++j) { // 60 UTXO --> 19,8 WCN total --> 60 × 272 WU = 16320 WU
                add_coin(available_coins, wallet, CAmount(0.33 * COIN), CFeeRate(0), 144, false, 0, true);
            }
            for (int i = 0; i < 10; i++) { // 10 UTXO --> 20 WCN total --> 10 × 272 WU = 2720 WU
                add_coin(available_coins, wallet, CAmount(2 * COIN), CFeeRate(0), 144, false, 0, true);
            }
            return available_coins;
        });
...
BOOST_AUTO_TEST_CASE(check_max_weight)
{
    ...
    {
        // Scenario 1:
        // The actor starts with 1x 50.0 WCN and 1515x 0.033 WCN (~100.0 WCN total) unspent outputs
        // Then tries to spend 49.5 WCN
        // The 50.0 WCN output should be selected, because the transaction would otherwise be too large
        ...
        BOOST_CHECK(result);
        // Verify that only the 50 WCN UTXO was selected
...

    {
        ...
        // The actor starts with 400x 0.0625 WCN and 2000x 0.025 WCN (75.0 WCN total) unspent outputs
        // Then tries to spend 49.5 WCN
...

    {
        ...
        // The actor starts with 1515x 0.033 WCN (49.995 WCN total) unspent outputs
