...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<WidecoinUnit>("WidecoinUnit");
#else
    qRegisterMetaType<WidecoinUnit>("WidecoinUnit");
#endif
}
...
bool WidecoinApplication::createOptionsModel(bool resetSettings)
{
...
void WidecoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    ...
    connect(window, &WidecoinGUI::quitRequested, this, &WidecoinApplication::requestShutdown);
...
void WidecoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
void WidecoinApplication::startThread()
{
    ...
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &WidecoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &WidecoinApplication::handleRunawayException);
    connect(this, &WidecoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &WidecoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}
...
void WidecoinApplication::InitPruneSetting(int64_t prune_MiB)
{
...
void WidecoinApplication::handleNonFatalException(const QString& message)
{
...
bool WidecoinApplication::event(QEvent* e)
{
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6-7. Parse widecoin.conf, determine network, switch to network specific
