...
/** Call node.updateRwSetting() with Widecoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Widecoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/widecoin/widecoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because widecoin 22.x will not try to read these.
...
//! Convert enabled/size values to widecoin -prune setting.
...
//! Get pruning enabled value to show in GUI from widecoin -prune setting.
...
//! Get pruning size value to show in GUI from widecoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayWidecoinUnit")) {
        settings.setValue("DisplayWidecoinUnit", QVariant::fromValue(WidecoinUnit::WCN));
    }
    QVariant unit = settings.value("DisplayWidecoinUnit");
    if (unit.canConvert<WidecoinUnit>()) {
        m_display_widecoin_unit = unit.value<WidecoinUnit>();
    } else {
        m_display_widecoin_unit = WidecoinUnit::WCN;
        settings.setValue("DisplayWidecoinUnit", QVariant::fromValue(m_display_widecoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if widecoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_widecoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<WidecoinUnit>() == m_display_widecoin_unit) return;
    m_display_widecoin_unit = new_unit.value<WidecoinUnit>();
    QSettings settings;
    settings.setValue("DisplayWidecoinUnit", QVariant::fromValue(m_display_widecoin_unit));
    Q_EMIT displayUnitChanged(m_display_widecoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/widecoin/widecoin/pull/8273
    ...
    // (https://github.com/widecoin-core/gui/issues/567).
