...
signature file SHA256SUMS.asc from widecoincore.org and widecoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/widecoin-core/guix.sigs/tree/main/builder-keys
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Widecoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"widecoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by widecoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the widecoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes widecoin.org lags behind widecoincore.org.)')
    )
