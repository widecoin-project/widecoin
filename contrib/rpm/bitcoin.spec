...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 1605
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 1604
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 29706
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 29707
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 39706
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 39707
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 1605
	%{_sbindir}/semanage port -d -p tcp 1604
	%{_sbindir}/semanage port -d -p tcp 29706
	%{_sbindir}/semanage port -d -p tcp 29707
	%{_sbindir}/semanage port -d -p tcp 39706
	%{_sbindir}/semanage port -d -p tcp 39707
	for selinuxvariant in %{selinux_variants}; do
