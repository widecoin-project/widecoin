...
The RPM spec file provided here is for Widecoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://widecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Widecoin git repository](https://github.com/widecoin/widecoin) but are not part of
...
Currently only `Source10` is used, for the example `widecoin.conf` file.
...
in the widecoin git repository. At present only an SVG version of the Widecoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the widecoin
...
needed when 0.12.1 is released, a proper fix is already in the Widecoin git
master branch.
...
idea to build Widecoin Core against a static version of that library compiled
...
The `widecoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `widecoin-server` package. At this point, most RPM based
...
The `widecoin-server` package should have SELinux support. How to properly do
...
Widecoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/widecoin/). He has been
packaging Widecoin for Fedora at least since 2012.
...
4. I renamed the `widecoin` package that contains the Qt GUI to `widecoin-core` as
that appears to be how the general population refers to it, in contrast to
`widecoin-xt` or `widecoin-classic`. I wanted to make sure the general population
