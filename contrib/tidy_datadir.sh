#!/bin/bash
# Copyright (c) 2013 The Widecoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Widecoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Widecoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Widecoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Widecoin 0.7 datadir."
    ;;
  3)
    echo "Detected Widecoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Widecoin 0.8 datadir."
    ;;
esac
